---
title: Install toolkit on IBM Cloud VPC infrastructure
---
...
infrastructure](https://cloudnativetoolkit.dev/getting-started-day-0/install-toolkit/ibm-cloud-on-vpc/ibm-cloud-on-classic)
...
`jq` can be found on the [<Globals name="shortName" /> prerequisites](https://cloudnativetoolkit.dev/getting-started/prereqs/) page
...
3. Update the value for the `ibmcloud.api.key` property in `credentials.properties` with your <Globals name="ic" /> API key
    ...
    **Note:** The API key should have been set up during [plan installation](https://cloudnativetoolkit.dev/admin/plan-installation/).
...
Once the Terraform has completed, walk through the [post installation steps](https://cloudnativetoolkit.dev/getting-started-day-0/post-installation/) to
