...
You can provision Hyper protect Crypto Services using IBM cloud catelog or using terraform script:

![hpcs-1](images/hpcs-1.png)
![hpcs-2](images/hpcs-2.png)

[Provisioning Hyper protect Crypto service from the IBM Cloud console and IBM Cloud CLI ](https://cloud.ibm.com/docs/hs-crypto?topic=hs-crypto-provision)
...
We are explaining step 2 in this tutorial. The following steps must be followed to complete the initialization 
...
```
## Here is the command to initialize(Key ceremony) HPCS using recovery crypto unit
...
[Use recovery crypto unit](https://cloud.ibm.com/docs/hs-crypto?topic=hs-crypto-initialize-hsm-recovery-crypto-unit)

## Create root or standard key

![add-key](images/add-key.png)

1) From the IBM Cloud console, click Key management service keys > Add key.
2) To create a new key, select Create a key.
3) Provide keytype (Root or Standard).
4) Provide KeyName.
...
[Provisioning Hyper protect DBaaS for MongoDB from the IBM Cloud console](https://cloud.ibm.com/docs/hyper-protect-dbaas-for-mongodb?topic=hyper-protect-dbaas-for-mongodb-create-service)
...
[Provisioning Hyper Protect Virtual Servers from the IBM Cloud console](https://cloud.ibm.com/docs/hp-virtual-servers?topic=hp-virtual-servers-provision)
